...
export const server = setupServer(
  http.get("https://acg.rip/page/1.xml", async () => {
    return HttpResponse.arrayBuffer(
      await fs.readFile("assets/test/acgrip.rss.xml")
    );
  }),
  http.get("https://share.dmhy.org/topics/rss/rss.xml", async () => {
    return HttpResponse.arrayBuffer(
      await fs.readFile("assets/test/dmhy.rss.xml")
    );
  }),
  http.get("https://nyaa.si/", async () => {
    return HttpResponse.arrayBuffer(
      await fs.readFile("assets/test/nyaa.rss.xml")
    );
  }),
  http.get("https://www.acgnx.se/rss.xml", async () => {
    return HttpResponse.arrayBuffer(
      await fs.readFile("assets/test/acgnx.rss.xml")
    );
  })
);
