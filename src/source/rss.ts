...
import { type Item, Source, USER_AGENT } from "./index.js";
...
export class RssSource extends Source {
	...
	public getTorrentUrl(item: RssItem): string {
		if (!item.enclosure)
			throw new Error(`item missing enclosure: ${JSON.stringify(item)}`);
		return item.enclosure.url;
	}

	public getSourceUrl(item: RssItem): string {
		if (!item.link)
			throw new Error(`item missing link: ${JSON.stringify(item)}`);
		return item.link;
	}
	...
	public override async getPage(page: number): Promise<Item[]> {
		...
		return (
			feed?.items.map((item) => {
				if (!item.title)
					throw new Error(`item missing title: ${JSON.stringify(item)}`);
				if (!item.pubDate)
					throw new Error(`item missing pubDate: ${JSON.stringify(item)}`);
				return {
					title: item.title,
...
export class NyaaRssSource extends RssSource {
	...
	public override getSourceUrl(item: Parser.Item): string {
		if (!item.guid)
			throw new Error(`item missing guid: ${JSON.stringify(item)}`);
		return item.guid;
	}
